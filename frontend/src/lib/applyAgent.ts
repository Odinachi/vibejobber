 ...
 */
export async function requestAgentApplyJob(jobId: string): Promise<{ runId: string; ok: boolean; error?: string }> {
  ...
  return { ok: data.ok === true, runId: (data as { runId?: string }).runId ?? "", error: data.error };
}
